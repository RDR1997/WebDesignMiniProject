...
if (count($_POST) > 0) {
    $result = mysqli_query($db, "SELECT *from admin WHERE ausername='" . $_SESSION["ausername"] . "'");
    $row = mysqli_fetch_array($result);
    if ($_POST["currentPassword"] == $row["apassword"]) {
        mysqli_query($db, "UPDATE admin set apassword='" . $_POST["newPassword"] . "' WHERE ausername='" . $_SESSION["ausername"] . "'");
